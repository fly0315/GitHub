...
* Includes
******************************************************************************/
//#include "VT4_Device.h"     // DSP28 Headerfile Include File
#include "Fr_UNIFIED.h"
#include "Fr_UNIFIED_cfg.h"
//#include "defineAddress.h"
	#include "AddrDef.h"
/*
#pragma CODE_SECTION(Fr_init, "flashfuncs");
#pragma CODE_SECTION(Fr_set_configuration, "flashfuncs");
#pragma CODE_SECTION(Fr_buffers_init, "flashfuncs");
#pragma CODE_SECTION(Fr_leave_configuration_mode, "flashfuncs");
#pragma CODE_SECTION(Fr_start_communication, "flashfuncs");
#pragma CODE_SECTION(Fr_send_wakeup, "flashfuncs");
#pragma CODE_SECTION(Fr_get_wakeup_state, "flashfuncs");
#pragma CODE_SECTION(Fr_get_POC_state, "flashfuncs");
#pragma CODE_SECTION(Fr_get_global_time, "flashfuncs");
#pragma CODE_SECTION(Fr_enable_interrupts, "flashfuncs");
#pragma CODE_SECTION(Fr_interrupt_handler, "flashfuncs");
#pragma CODE_SECTION(Fr_start_timer, "flashfuncs");
#pragma CODE_SECTION(Fr_stop_timer, "flashfuncs");
#pragma CODE_SECTION(Fr_check_tx_status, "flashfuncs");
#pragma CODE_SECTION(Fr_check_cycle_start, "flashfuncs");
*/
/******************************************************************************
* Constants
******************************************************************************/
                       ...
                       const Fr_low_level_config_type *Fr_low_level_config_temp_ptr)
{
    ...
    temp_value = Fr_CC_reg_ptr[FrMVR];
                                    ...
                                    uint8 *Fr_data_length_ptr,uint16 *Fr_slot_status_ptr)
{
    ...
    uint16 Fr_p;                                // Temporary counter used for copying
	uint16 Fr_buffer_index;//20120921cwr
    ...
    if(temp_value_2 & FrMBCCSR_LCKS)            // Is the MB locked?
    {
        ...
        // Load current MB index
        Fr_buffer_index = Fr_CC_reg_ptr[FrMBIDXR0 + Fr_MB_registers_offset_add_temp];
        // Calculate the message buffer header 
        header_MB_ptr = ((volatile uint16 * FR_DATA_FAR)(Fr_HW_config_ptr->CC_FlexRay_memory_base_address) + Fr_buffer_index * 5);
...
void Fr_enable_interrupts(uint16 Fr_global_interrupt, uint16 Fr_protocol_0_interrupt, uint16 Fr_protocol_1_interrupt)
{
	...
	//	temp_value_1 |= FrGIFER_MIE;          ///ZLZLZLLLLLZLZLZ
...
void Fr_interrupt_handler(void)
{
   ...
   // Fr_CC_reg_ptr[FrGIFER] = ( Fr_CC_reg_ptr[FrGIFER]&(~FrGIFER_MIE));
	//  Fr_disable_interrupts((FrGIFER_MIE ),0, 0);
...
//	Fr_enable_interrupts((FrGIFER_MIE ), (0), 0);
...
Fr_tx_status_type Fr_check_tx_status(uint8 Fr_buffer_idx)
{
    ...
    else
    {
        ...
        {   // MB works in the STATE transmission mode, DVAL bit has to be check
            ...
            {
                //return FR_NOT_TRANSMITTED;
				return FR_TRANSMITTED_CWR;//20120831 cwr
            }
        }
...

{
	...
	// If DUP == 1 : Frame header and MB data field updated
	if(temp_value_1 & FrMBCCSR_DUP)   // Yes, Valid non-null frame received 
	{         
  ...
  {  
/*
		if((fr_CNT_A[0]>=60)&&(fr_CNT_B[0]>=60)&&(Fr_buffer_idx==2))
    	{
    		DSP2_FLEX_AB |= 0x03;
    		DSP2_FLEX_STATUS |=BIT0;
		}
		else if(((fr_CNT_A[1]>=60)&&(fr_CNT_B[1]>=60))&&((Fr_buffer_idx==3)||(Fr_buffer_idx==4)))
		{	
			DSP2_FLEX_AB |= 0x0c;
			DSP2_FLEX_STATUS |= BIT1;	
		}
		else if(((fr_CNT_A[2]>=60)&&(fr_CNT_B[2]>=60))&&((Fr_buffer_idx==5)||(Fr_buffer_idx==6)||(Fr_buffer_idx==7)))	
		{
			DSP2_FLEX_AB |= 0x30;
			DSP2_FLEX_STATUS |= BIT2;	
		}
		else if(((fr_CNT_A[3]>=60)&&(fr_CNT_B[3]>=60))&&((Fr_buffer_idx==8)||(Fr_buffer_idx==9)||(Fr_buffer_idx==10)))	
		{
			DSP2_FLEX_AB |= 0xc0;	
			DSP2_FLEX_STATUS |= BIT3;
		}
*/
		//总线断开
